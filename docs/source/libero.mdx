...
### Object overlays (replace / add without editing stock LIBERO)

By default, LIBERO evaluation is unchanged. To **replace** or **add** objects
without modifying the installed LIBERO assets/BDDL, pass `--env.overlay` to a
YAML file. Full how-to-run (including how to look up BDDL category names) is in
[`examples/libero_overlays/README.md`](https://github.com/huggingface/lerobot/tree/main/examples/libero_overlays/README.md).

`replacements` keys are BDDL **categories** (the token after `-` in
`tomato_sauce_1 - tomato_sauce`), not instance ids. List them with:
...
print(bddl)
objs = bddl.read_text()
block = objs[objs.find("(:objects") : objs.find("(:obj_of_interest")]
for m in re.finditer(r"(\w+)\s+-\s+(\w+)", block):
    print(f"instance={m.group(1)}  category={m.group(2)}")
PY
```

`lerobot/smolvla_libero` expects `camera1` / `camera2` plus a padded slot, so the
commands below pass `--env.camera_name_mapping` and `--policy.empty_cameras=1`
(see [Rename Map and Empty Cameras](./rename_map)).

```bash
# Stock scene (identical command as before — no overlay)
uv run lerobot-eval \
...
# Replace tomato_sauce with a red cube
uv run lerobot-eval \
  --policy.path=lerobot/smolvla_libero \
  --env.type=libero \
  --env.task=libero_object \
...
# Add several distractors (red_cube + blue_cube + a second red_cube)
uv run lerobot-eval \
  --policy.path=lerobot/smolvla_libero \
  --env.type=libero \
  --env.task=libero_object \
...
Overlays copy the stock BDDL, patch a temp file, and optionally register custom
object classes. Under `mode: add`, list as many `add_objects` entries as you need
(same category twice requires unique `instance` + `region_name`). `mode: add`
disables fixed init states by default (MuJoCo state layout changes). Never edit
files under the LIBERO package install.
