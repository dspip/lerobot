...
def test_holds_previous_action_during_fault():
    ...
    np.testing.assert_array_equal(outs[2], [[20, 20]])
    np.testing.assert_array_equal(outs[3], [[20, 20]])
    np.testing.assert_array_equal(outs[4], [[20, 20]])
    np.testing.assert_array_equal(outs[5], [[60, 60]])
...
def test_resumes_after_duration():
    ...
    assert inj.apply(_action(1, 3, 2.0))[0, 0] == 1.0
    assert inj.apply(_action(1, 3, 3.0))[0, 0] == 3.0
...
def test_new_episode_can_trigger_independently():
    ...
    inj.apply(_action(1, 1, 2.0), episode_ids=[0])
