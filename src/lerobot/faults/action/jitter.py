...
"""Action-jitter fault injection (additive Gaussian noise on actions)."""
...
class ActionJitterFault:
    """Add independent Gaussian noise to each action dimension every step."""
    ...
    def __init__(
        ...
        if config.type != "action_jitter":
            raise ValueError(f"ActionJitterFault requires type='action_jitter', got {config.type!r}.")
        ...
        self._rngs = [self._make_rng(i) for i in range(num_envs)]
        self._states = [_EnvJitterState() for _ in range(num_envs)]

    def _make_rng(self, env_idx: int) -> np.random.Generator:
        return np.random.default_rng(self._base_seed + env_idx)
        ...
        for i in indices:
            ...
            if isinstance(episode_ids, dict):
                ...
                if len(episode_ids) == len(index_list):
                    ...
                    ep_id = episode_ids[i]
            self._states[i] = _EnvJitterState(episode_id=ep_id)
            self._rngs[i] = self._make_rng(i)
    ...
    def notify_dones(self, dones: np.ndarray) -> None:
        ...
        for i, done in enumerate(dones):
            if done:
                ...
                self._states[i] = _EnvJitterState(episode_id=ep_id, finished=True)
    ...
    def apply(
        ...
        for env_idx in range(self.num_envs):
            ...
            noise = self._rngs[env_idx].normal(0.0, self.config.noise_std, size=proposed.shape)
            jittered = proposed + noise
            executed[env_idx] = jittered
        ...
        self.event_logger.log(
            {
                ...
                "executed_jittered_action": jittered.astype(float).tolist(),
                "noise": noise.astype(float).tolist(),
            }
        )
